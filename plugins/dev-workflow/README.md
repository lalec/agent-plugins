...
| `/blueprint` | Plan on the strongest model, run on the cheaper ones. Give it a goal — or the id of a roadmap item that is only a name — and it reads what the project already knows, checks every file it means to touch actually exists, decides everything an executor would otherwise have to guess, and writes the result to the roadmap as one umbrella plus child items sized for `/pilot`, each with its lane, its dependencies and the checks that prove it. It edits nothing else; the last line is the `/pilot --items …` that runs the plan. |
...
```
A goal handed straight to `/pilot` gets decomposed at the gate, on whatever model the session runs, with no time to read the record — so a plan written as prose steps becomes one oversized task, and the agent building it ends up choosing the model, the threshold and the budget itself. This lane does the planning first, on the strongest model, and writes the plan where `/pilot` reads it: one umbrella item carrying the origin, the context, and a numbered *decisions made here so the executor does not* block, plus child items each small enough for one unattended run — its lane, what it depends on, whether Sonnet or Opus should build it, the exact place in the code it lands (grepped, so a reference that doesn't exist is never written), what *done* means, and the checks with concrete expected values. A mechanism the code already has is named for reuse rather than rebuilt; a lever the log shows was tried and reverted is refused unless there's new evidence. Decisions that are yours — money, product tradeoffs — come to you once, at the single gate, then get written as decided; anything an executor finds undecided it reports rather than guesses. Its only write is the roadmap, committed and not pushed, and it ends with the `/pilot --items … --no-push` that runs the plan in dependency order — or `/code <first item>` if you'd rather sit in on the first one. `/pilot` given the umbrella's id expands it to the open children and hands each one the plan's decisions; the umbrella closes itself when the last child does.
