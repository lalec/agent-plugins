...
- [ ] `whats-up.md` reads six universal stores plus any declared via a `whats-up-store:` frontmatter key (each with a stated fallback), calls `open-deferrals --with-fail`, reconciles a `parked` gate against later evidence before reporting it, states that it never writes, and reports per `code.md § Done` with the ranked `Open` table as its plan — **no** separate numbered action list
