...
- `**Verification (umbrella):**` — the whole-plan check and the **stop rule**: the measurement that, going the wrong way, halts the plan and files an `**Updated:**` line here. **A stop rule measures the plan's own delta, never accumulated state.** Write it as a before/after over the same corpus, taken by the run that trips it — "how many rows *this plan* changed", not "how many rows differ". A rule written against accumulated state trips on drift that was already there: one real umbrella's stop rule fired on pre-existing coordinate drift, blocked its own last two children, and cost a day until the user amended it to measure the mission's delta. If a delta is genuinely unmeasurable, the stop rule is a **question for the user**, not a threshold the run will trip over
