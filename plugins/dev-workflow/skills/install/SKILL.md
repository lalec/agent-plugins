...
- [ ] `.claude/commands/audit.md` carries both a `pilot-lane:` and a `whats-up-store:` block, routes the triage rule to `<PREFIX>-review/references/security-review.md § Triage` instead of restating it, and caps Step 1 at 10 candidates per run; `<PREFIX>-review/references/vex.yaml` exists and `governed-paths.conf` carries `DEPENDENCY_MANIFESTS` plus the `vex.yaml` `EXEMPT` row
