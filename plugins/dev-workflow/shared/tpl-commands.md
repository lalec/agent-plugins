...
**Usage:** `/pilot <goal>` — a batch of work (e.g. open roadmap items matching a filter) or a target state to reach (improve an area until measurable criteria pass). Flags: `--max-tasks N` caps the run (default 10); `--items <id>,<id>` runs exactly those roadmap items (how `/roadmap` hands over a set); `--gates [<name>,<name>]` re-measures parked verdicts and settles them (all of them, or just the named ones); `--prod` pre-answers Ship; `--no-push` pre-answers Hold and skips the close-out push; `--grant <unit>=<N>` (repeatable) pre-answers Budget for a metered lane; `--regression full|auto` opts the whole mission out of the fixed `smart` scope.
...
- Reversible decisions are made autonomously with the recommended default and labeled `auto-decided (pilot run — not user-confirmed)` in every downstream record — never presented as user consent. This extends to **concluding an open claim**: a task that re-measures one states a verdict from the ladder in Step 2, and the run itself concludes every verdict that takes no irreversible action. What separates the two is not who raised the question but what the answer does.
...
- **A store of open claims** (`--gates` today): the flag names a store, the store's query returns the claims, and each returned claim is one task. Nothing else about the mission changes. A **claim** is a record plus the condition that would resolve it — a parked verdict, and later a deferral or a stalled item; they decay the same way, so they decompose the same way and this bullet takes each new store as another row, never as a second mechanism.
  ...
  Each task **re-measures the claim's condition and states one verdict from the ladder in Step 2** — that is its acceptance statement. The task is read-only, so it needs no serve-env and no QA cycle; route it to the lane that owns whatever the condition measures, else the lane named in the claim's own log entry, else `pipeline`, and let it exit at that lane's declared `close-out`. **Claims whose refreshed evidence is the same set are one task, and that task answers every one of their names verbatim** — one repair is routinely parked under two names, and one lane routinely parks the same name once per run; answering only the name you happened to read leaves the others parked forever. A named claim the query does not return is reported at the gate and dropped, never guessed at.
...
**Unattended runs take the top two rows and nothing else.** A run with nobody present concludes what the measurement concludes and carries every `drifted` / `live` / `judgment` claim forward untouched, to the next run that has a person in it. They are not defaulted, not skipped, and not recorded as decided — the batched gate in Step 3 is where they land.
...
**Closing line — staged by clearing cost.** `code.md § Done` derives the line from the Open rows, and its table assumes one command closes them. This lane routinely holds three kinds at once, so stage them by what it costs to clear one: **decisions** (a parked gate, an acknowledgement — minutes, no code) → **repairs** (a recorded `fail`, a gated data repair — one lane each) → **items** (in-progress work — a mission). Rows all in one stage → that stage's batched command, per the batch rule; for decisions that is `/pilot --gates`, which re-measures each one before proposing anything and closes the ones the measurement kills without asking. Rows spanning stages → `Next:` names **stage one only**, with a second `Then:` line showing the path. **Never propose a stage a predecessor would dissolve:** a gate that releases two in-progress items changes what the mission should contain, so clear it first and say so rather than batching both into one line that will be wrong by the time it runs.
```
