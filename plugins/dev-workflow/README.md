...
Everything else is written automatically, inside your repo. Two things sit outside it and are offered rather than done: scheduling headless shifts, and letting runs see how much of your Claude allowance is left. The second wraps your status line so the figures it already receives land in a file `/pilot` can read — your own status-line script isn't edited, and deleting one line from your settings undoes it. Say no and it prints the change for you to make yourself. It's per-machine, not per-project, so the next project you install into finds it already there.
