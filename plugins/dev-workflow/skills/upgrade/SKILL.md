...
- [ ] `<PREFIX>-pm.md` is missing step 1.5 (prod deploy via `<PREFIX>-deploy` with `target=prod`)
...
- [ ] `<PREFIX>-pm.md` step 1.5 (Deploy to prod) does not contain "You do not ask the user"
...
- [ ] `.claude/commands/code.md` lacks Step 0.5 (capture acceptance criteria + regression flag) and Step 1.5 (persist verifications post-dev)
- [ ] `.claude/commands/fix.md` lacks Step 0.5 (capture) and Step 2.5 (persist verifications post-dev)
...
- **Insert `<PREFIX>-pm.md` step 1.5** (prod deploy) from `../../shared/tpl-agents.md § tosk-pm` between step 1 (Verify QA phases) and step 2 (Write delivery log). Idempotent — skip if a step 1.5 referencing `<PREFIX>-deploy` already exists.
...
- **Tighten `<PREFIX>-pm.md` step 1.5 (Deploy to prod)** with the version from `../../shared/tpl-agents.md § tosk-pm` step 1.5 (adds "You do not ask the user" and "Skip silently"). Idempotent — skip if step 1.5 already contains "You do not ask the user".
...
- **Scaffold test reference files** (no confirmation — absent, not stale): create `.claude/skills/<PREFIX>-test/references/custom-tests.yaml` (`tests: []` + leading comment) and `references/custom-tests.md` from `../../shared/tpl-lifecycle.md § tosk-test` if missing. Idempotent — skip files that exist.
...
- **Insert Step 0.5 + persist step in `code.md` and `fix.md`**: from `../../shared/tpl-commands.md § /code` and `§ /fix`, add Step 0.5 (capture acceptance criteria + regression flag — read-free) and the persist step (`code.md` Step 1.5, `fix.md` Step 2.5); route Step 0 confirm "Yes" to "continue to Step 0.5"; pass the captured assertions to `<PREFIX>-dev` as acceptance criteria and forward `regression_mode` + new-entry names + changed paths to `<PREFIX>-qa` (preserving `regression_mode` across the `mode=retest` loop). Identical Step 0.5 wording in both files (Rule 3). Idempotent — skip each file if it already contains "What should be verified before this ships".
...
- `<PREFIX>-pm.md` contains a step 1.5 invoking `<PREFIX>-deploy` with `target=prod`
...
- `<PREFIX>-pm.md` step 1.5 contains "You do not ask the user"
