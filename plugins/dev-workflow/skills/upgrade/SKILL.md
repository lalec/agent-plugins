...
- [ ] `<PREFIX>-deploy/SKILL.md` frontmatter lacks `disable-model-invocation: true` — deploy must not be model-auto-invocable; it runs only via the `<PREFIX>-dev` deploy step or `/code|/fix --prod` (explicit invocation is unaffected)
...
- **Gate `<PREFIX>-deploy` against model invocation**: add `disable-model-invocation: true` to `<PREFIX>-deploy/SKILL.md` frontmatter if absent. Explicit invocation by `<PREFIX>-dev` (deploy step) and `/code|/fix --prod` is unaffected; this also stops the deploy skill being auto-preloaded into subagents. Idempotent — skip if already present.
...
- `<PREFIX>-deploy/SKILL.md` frontmatter contains `disable-model-invocation: true`
