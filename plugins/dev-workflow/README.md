...
| **How wide it goes** | Decided after the code is written, from what actually changed: a change crossing two domains, or landing where a check is already failing, pulls in the full suite. `--regression full` forces it — and because that re-covers every earlier check on every task *and* every fix round, a multi-task run tells you the scope up front, next to the task list, with what it costs in tokens while you can still change it. The number that matters is how many earlier checks each task drags in, not how many tasks there are: work landing in a file every check already touches pulls the whole suite in whatever scope you picked. |
