...
| The one check that proves a journey could be deferred like any other (`signed-off-with-deferrals`), and repeat deferrals were report-only — one missing test environment blocked five checks across three deploys, each run noting it and moving on → end-state deferrals are `blocked`, and 3+ deferrals on one blocker asks to fix the blocker first | this pass |
