...
**The last line.** After the five blocks, print one line. It is **derived from the Open rows**, never authored separately — so it can never disagree with the table above it. Count the **actionable** rows: a row is actionable when *something can be done about it now* — a command would do it, or it needs a decision **and this run stated which one it would take**. A row where nothing can be done (a bare count, a fact) is not actionable, and neither is a decision the run could not recommend. Then:

| Actionable Open rows | The line |
|---|---|
...
**A parked gate this session created outranks all of it**: `Next: resume here — <what it awaits>`, the only case where this session must survive. That is a gate awaiting an answer *right now*, not a gate the log records as parked — those are ordinary Open rows and batch like any other. On the 2nd+ pipeline run of a session, append `— in a fresh session`; long sessions degrade quality and it does not need a paragraph.
...
- Too long? Fold the rows in a good state into one (`N done · M proven`) and keep every other row as it is. **Open is never folded; Emerged folds only its low rows, into one line that still names every id.** If the report does not fit a screen, rows are being written as paragraphs.
- **Nothing after the blocks, and nothing between them.** Every sentence that wants to follow the closing line passes one test: it changes what the reader would *do* → an Open row, with the run's recommendation as the last clause of `Why it is open`; it changes what they *believe* → a Learned bullet, inside the cap of three; neither → cut. "Two things worth knowing", "one judgment call you may want to overturn" are this rule broken by name — a judgment call the run made is an Open row recommending it stand, so the reader overturns it in one word instead of reading a paragraph about it.
...
whats-up-store:
  reads: .claude/pilot/last-run.json — the newest run's marker (ts, mode, verdict, runnable_left) — .claude/pilot/running, present while a run is in flight, and ~/.claude/usage-snapshot.json, the account's allowance figures a run reads before starting work
  healthy: last-run.json absent → n/a (no run has happened yet); else its verdict is not `failed` and either its ts is within 2 h, or its runnable_left is 0 **and the roadmap and unproven-work stores this reader just read are themselves empty** — a marker's 0 is a claim about the moment it was written, and work landing since is exactly when a loop that stopped firing still reports healthy, which is the failure this store exists to name. Also: usage-snapshot.json exists and its written_at is within 24 h — absent or older means the status-line wrapper that keeps it current has come undone, so every run from now on reports its allowance as unknown, correctly and forever, and nobody notices; `Next` is the install step that re-wires it. Nothing gates on this, here or anywhere: a check-in is simply where a person is present to fix it
...
**Allowance:** read `~/.claude/usage-snapshot.json` — the status-line wrapper writes the account's own figures there, and it is the only place Claude Code publishes them. Report one line: each window as percent used with the time it resets. **Absent, unparseable, or `written_at` more than 3 minutes old → `allowance: unknown`, said out loud, and the run proceeds** — a mission that halts because a status-line script did not run halts for no reason, and a fresh session has none until its first API response. A `resets_at` already in the past means the percentage beside it is dead: report `window reset since this was written` and treat it as unknown, never as a low number. Report only; the decision is Step 2's, on measured evidence. Nothing here gates. When the run is about to start 3+ tasks at a terminal, add: `caffeinate -i` prevents the machine sleeping through a reset, which is the branch that waits for a keypress instead of continuing.
...
pilot-lane:
  ...
  apply: <optional — how an approved verdict from this lane is executed, and the non-mutating
          preflight that proves the run can execute it>
  reversal: <optional — what the apply writes first that undoes it (a manifest path, a snapshot)
             and the command that replays it; declaring it makes the apply reversible>
```

**`apply:` is what turns an answered verdict back into work.** A lane that parks a verdict and declares no `apply:` can produce evidence and never act on an answer — which is correct for a lane whose measuring half is deliberately read-only, and is exactly why the route has to be declared separately rather than assumed. Report a missing `apply:` at the gate, next to the recommendation it blocks, instead of discovering it after the user has answered. It is also the bound on an unattended run: **no `apply:` means that lane may only observe.**

**`reversal:` is what lets the run apply without asking.** The autonomy contract already makes reversible decisions the run's own; what was missing was a way for a lane to say its apply *is* reversible. A lane declaring both `apply:` and `reversal:` has one, so a **`live`** verdict from it is applied by the run at Step 3 `(a0)` — preflight, reversal artifact written first, apply, outcome recorded `(pilot-auto)` with the reversal path in the Status row — with no question, attended or unattended. `drifted` stays a stop and `judgment` stays the user's; the declaration moves only `live`, the verdict where the re-measure confirmed the proposal and the only thing left was permission to do what was already proposed. A reversal nobody can replay is irreversibility under a reversible label, so the line names the replay command or is left out.
...
**State the regression scope in that question, not just the tasks — with its price, in tokens.** Tokens are the unit a session's usage limit is measured in, so a figure in dollars is one the user has to convert before it can change their answer. Regression scope is the one setting that moves mission spend by an order of magnitude: `full` re-covers every prior verification per task *and* on every fix cycle, so `--regression full` on a multi-task mission is a different order from the pinned `smart` default. Quote both numbers while the user can still change them:

- `full` measured **8–17×** a `smart` mission of the same size (two real missions, 2026-08-27).
- `smart` measured **~70M tokens per task** — a 10-task mission at 714M, 97% of it cache reads, 5,631 requests, enough to spend a usage limit mid-run (jobzeeker, 2026-09-18).

**Cost tracks the prior count, not the task count.** That mission's tasks pulled 33–91 priors each because they landed on hub files every prior already covers; the same ten tasks on leaf files would have pulled a fraction. So say what the scope is *worth here*: name the per-task prior count if a `blast` from Step 1 already returned it, and otherwise say the figure assumes hub-file breadth. A fix cycle re-walks the whole set, so a mission whose tasks are likely to need one prices as more than its task count suggests. When `.claude/pilot/last-run.json` carries a `per_task_cost` measured on **this** project under the same regression scope (Step 2 records one per task), quote that instead — it is the only figure that describes this codebase rather than another one; a figure stamped with a different scope is quoted as indicative and labelled so. `.claude/pilot/state.json` holds a headless shift's own totals, and `session-cost.py` prints the token line for any finished run.

**Put Step 0's allowance line in the question too**, verbatim, beside the price — `unknown` included. The person is present exactly once, here, and this is the one moment a mission that will not fit can be made smaller instead of stopping halfway. Pricing the run while they can still change it is the same reason **Budget** is asked up front. Report both; never gate on either.
...
**Stop between tasks, never inside one.** Read `~/.claude/usage-snapshot.json` before and after each task; the difference is what that task actually cost, in the unit the allowance is measured in. Before dispatching the next task, compare what is left in each window against what the last task took:

| Condition | What the run does |
|---|---|
| The 5-hour window has less left than the last task took | **Stop the mission here.** Close out per Step 3 over the tasks that finished, state the reset time, and make the closing line `/loop 30m /pilot --items <the ids still open> --max-tasks 1 --no-push` — closed ids are skipped by the Decompose bullet, so it resumes exactly what remains and idles once done |
| The 7-day window has less left than the last task took | **Stop and say when it returns.** No loop: one idling for days is a worse answer than a sentence |
| Snapshot unknown, or the measured cost was 0 | Carry on. Unknown is never read as empty, and a window that moved less than its own resolution says nothing |

Record each task's measured cost **as that task finishes** — not at close-out — into `.claude/pilot/last-run.json` as `per_task_cost`, stamped with the commit and the `regression_mode` it was measured under. A run killed mid-mission then still leaves what it learned, and Step 1's price quote gets a figure measured on *this* project instead of another one. A figure stamped with a different scope is quoted as indicative and never used to stop a run.

**Why stop rather than push on.** A mission that dies mid-task leaves a half-reviewed change, a running server and no log entry, and the recovery is a person reconstructing what happened. A mission that stops between tasks leaves a closed-out record and one command. This is also what keeps the *next* limit hit recoverable: when a wide fan-out dies all at once, every child's death reaches the session as its own turn and each is refused in turn — and Claude Code re-arms its own wait at most twice in a row before giving up for that window. Five refusals in 27 seconds is how a real mission burnt that budget on death notices and then sat idle for 2h19m after the reset.
...
The rule that decides who concludes is **not** who raised the question — it is whether the conclusion takes an irreversible action. Declining or closing writes nothing, so it is reversible: the claim can be parked again from the same evidence. Applying is not. A task that re-measures a claim therefore states exactly one verdict, and three of the five need no user at all:

| Verdict | What the re-measure found | Concluded by |
|---|---|---|
...
| `live` | re-measured, and the proposal still holds | **the user**, on a recommendation to approve — or **the run**, `(pilot-auto)`, when the lane declares `reversal:` (Step 1, *lane registry*): the apply is undoable, so this is a reversible decision |
...
**(a0) Settle the verdicts this run produced** — run when Step 2 left any `drifted`, `live` or `judgment` verdict. The autonomous ones are already closed and recorded. A `live` verdict from a lane declaring `reversal:` is applied **here first, without a question** — preflight, reversal artifact, apply, `(pilot-auto)` with the reversal path in Status — and joins them; this step is then the single sitting where the rest get answered.

**Preflight first, then ask.** For every recommendation that would write, run its lane's declared `apply:` preflight — non-mutating, and it proves the run can actually perform the write: credentials present, permission mode allows it, target reachable. A lane with no `apply:` fails this by definition. Preflight the route *before* spending the question, because a run that collects consent and then discovers it cannot act has spent the one expensive moment of the mission on nothing.

**Skip the question when nothing is left to ask.** No `drifted`/`live`/`judgment` verdict, or every one already put to the user at the mission gate with the re-measure changing neither the recommendation nor the reason for it → record and move on. Asking twice for one decision is the failure this step is most likely to produce, because `(a0)` runs minutes after the gate and the run remembers proposing something.

**On an unattended run, ask only the high or critical rows.** Those are: a `drifted` claim (the evidence moved — a stop); a row that releases or blocks a roadmap item whose `**Priority:**` is `high`; a task that failed or blocked twice in this run; and the 3+-on-one-blocker escalation. Ask them once, with the configured timeout, then park whatever went unanswered with the rest. Every other user-column row — a `live` proposal, a `judgment`, a medium-priority release — is parked verbatim in the log and listed in the report; the check-in's `/pilot --gates` is where those get asked, as one sitting. This is what lets a loop run for days without paging its owner for every judgment call, while never deciding one for them.

**Then one `AskUserQuestion`, whose question text is the disposition table** — one row per verdict: the claim, what the re-measure found, the recommendation, and where its evidence lives. Say in that text which rows the run will **apply** and which it can only **file**, with the preflight's reason. **Where the mission gate already showed the user something about a claim, lead that row with what the re-measure changed** — the number that moved, the assumption that turned out false. A second question the user cannot tell apart from the first reads as the run forgetting it already asked, even when the evidence behind it is genuinely new. Options: `Take all <N> recommendations (Recommended)` / `None — leave them parked`; the automatic "Other" is the per-row override channel. Timeout → everything stays parked and is reported as unanswered, never as consent; nothing is lost, because the autonomous closes already happened. Answering five claims is one decision session, and offering them one at a time is what turned five into a backlog.

**Record the answer before acting on it, and never on its own.** Write every answer into the close-out entry's `**Decisions:**` line under the claim's parked name, verbatim, *before* any approved apply runs — a session that dies mid-apply must leave the decision recorded, not lost with the transcript. And **an approval is recorded only together with the id of the work it authorises**: either this run applies it via the lane's `apply:` route and the entry cites the resulting commit, or the run files a roadmap item and the same entry carries `**Addresses:** <id>`. Never one without the other — an `approved` with no executable work behind it closes the claim while the work stays undone, which is worse than leaving it parked.

**Then apply, here, before (a).** For each approved row whose preflight passed, invoke the lane's `apply:` route at the top level, and record the outcome on the same entry. An approval that this run can execute is executed in this run — carrying it to a later session is what the filed form is for, not the default. A row whose apply fails mid-way stops there and is reported with what it did and did not do; it is never retried into a different shape.

**A filed item has to be executable cold.** Its body carries the approval and its date, the committed evidence path, and the exact parameters — everything a session with no memory of this one needs. That is what makes the next `/whats-up` enough, and it is why nothing here is ever handed over as prose.
...
**(a3) Docs + references, once, for everything no `<PREFIX>-pm` covered.** A pipeline task closes through `<PREFIX>-pm`, which runs the docs check itself; a **tweak-lane or discovered-lane task closes through neither**, so on a mission with no pipeline task nothing checks docs or references at all. Take the union of paths those tasks touched and the skills that own them, then: use `<PREFIX>-docs` to check staleness against that set, and `<PREFIX>-skill` for reference sync scoped to those skills. Skip only when every task ran the pipeline lane — and say so in the scorecard rather than leaving the row blank. **One sync for the mission, not one per task**: `/tweak` states the same rule for a burst, and the reason is identical — N tasks in one area produce one docs answer, and re-deriving it per task is the cost this lane exists to avoid.

Two things this reliably catches, both observed on the first real mission: a path a lane wrote to that `governed-paths.conf` does not cover (a repair lane's artifact directory), and a delivery-log or command-surface fact in `docs/` that the mission's own changes just dated.
...
| Docs | `<PREFIX>-docs` ran over the non-pipeline tasks' paths at (a3) — what it changed, or `n/a — every task ran the pipeline lane`. A blank row means the step was skipped, not that docs were clean |
| Ref sync | `Reference Sync:` fields from the pipeline handoffs, plus `<PREFIX>-skill`'s result from (a3) for every other task. On a mission with no pipeline task the handoff half is `n/a` and (a3) is the whole row — never report a Reference Sync that no agent produced |
| Allowance | each window's percent used at the last read with its reset time, and the per-task cost measured — or `unknown — no snapshot`. When Step 2 stopped the run, say so here with the window that stopped it; the ids it did not reach are **Open** rows, and the closing line is the resume command Step 2 named |
| Spend | per granted unit: `<spent>/<grant>` from the ledger, plus any lane closed on exhaustion |
| Verdicts | every claim the run touched now shows a non-`parked` decision, or is still returned by `open-gates` — grep the close-out entry for each name. A claim the run reports as settled that the query still returns is the failure this row exists to catch, and it is almost always a name that was not repeated verbatim |
| Approvals | each `approved` in `**Decisions:**` has either a cited commit or an `**Addresses:**` id in the same entry. An approval with neither is an unfinished repair wearing a closed gate |
| Models | `orchestrator / agents / children` as actually passed on the dispatch calls — `session / sonnet / sonnet` on an unattended run, `session / session / sonnet` on an attended one. This row is what makes an unattended run's cost and quality comparable with an attended one's |

**(d) Restore stashed WIP and clear the run marker.** If pre-existing WIP was stashed at Step 0, `git stash pop` it now; report any conflict instead of resolving it silently. Then write `.claude/pilot/last-run.json` — `{"ts", "mode": "standing|flagged|goal", "unattended": true|false, "verdict", "tasks": {"done", "failed", "skipped"}, "parked", "asked", "runnable_left", "models": {"orchestrator", "agents", "children"}}` — and remove `.claude/pilot/running`. It is a **run marker, not mission state**: the roadmap and the log remain the state, and this file only lets the next reader — `/whats-up`'s store row, a loop deciding whether to back off — see that a run happened and how it ended. If `PILOT_NOTIFY_URL` is set in the environment, `curl` the Verdict line, the Open table and the `Next shift` rows to it as one message (`{"content": "<text>"}` works for a Discord or Slack incoming webhook) — that is how an unattended run reaches a phone; unset means nothing is posted, silently.
...
`live:` is optional and exists for a store that can be in three states rather than two — a loop or a shift that is stale when it stopped firing and *busy* while a run is in flight. When the `live:` line matches, the store reports `busy` regardless of `when-bad`; when it does not, `when-bad` decides as before. A store with only two states declares no `live:` line.
...
- **A `parked` gate is a claim, not a state** — the log is append-only, so a gate answered weeks ago still reads `parked` in the entry that raised it. `open-gates` resolves this for you: a parking is closed by any newer decision on the same name, so a later entry answering it closes it and repeated parkings of one name each survive on their own. **Trust the projection and do not re-derive it.** Two cases still need you. A gate answered under a *different* name never closes — the log's own rule is that the answering entry repeats the name verbatim, so a plausible rename is a `Learned` bullet, not a silent merge. And on the grep fallback there is no projection at all: check each hit for a later entry deciding the same gate, a deploy that could only have happened if it was answered, or the change sitting in the tree.
- **A gate's evidence is a claim too, and it decides whether anyone can be asked anything.** `open-gates` returns each parking's `condition` — what it recorded as the thing that would answer it — and its `age_days`. Classify from those two, which costs one line of text and no file reads:
  - **The condition names a trigger that has not fired** ("wait for a set that trips the tolerance on its own", "needs one tagged example"). Nobody can answer this today. It is a **Status** row carrying the trigger, not a decision — reporting it as one every time anyone looks is how a handful becomes a wall.
  - **The condition is measurable and delivery entries have landed since it was parked.** The proposal rests on numbers that have had time to move, so the honest row is `unrecommendable — evidence <N>d old` and `Next` is `/pilot --gates <name>`, the re-measure. Recommending the parked proposal here is the one error that can do damage: a real one had drifted from 82 records to 105, and applying it as written would have destroyed 46 legitimate rows.
  - **No measurable condition** — the question is a preference, not a measurement. There is no evidence to go stale, so recommend it now, at any age.
...
- **An in-progress item may have stopped.** Over the **in-progress set only** — bounded, and the one set whose rows this report proposes work on. Take `last_addressed` from `roadmap-open` (the newest delivery-log entry citing its `**Id:**`); never cited, or 21+ days ago, marks the item **stalled** in its Open row with the day count. **Held is not stalled:** an item a parked gate releases, or one named by a recorded `fail` or `blocked`, is waiting on something this report already raises — say which, and do not call it stalled. Where the item's body asserts a checkable repo fact ("no tests exist", "the endpoint returns 404"), spot-check it; a body that is now false is a `Learned` bullet, competing for the existing 3-bullet cap rather than raising it. Started work that quietly stopped is invisible in every other store: it has no gate to park, no verification to fail, and its status line has said `in-progress` since the day it stopped.
...
**The ladder ranks the report; it does not filter it.** Evaluate every question, not just the ones before the first yes — stopping early decides the `Verdict` line and nothing else. A condition that is true but outranked still emits its rows into `Open`, below the winning verdict's rows: `gate-blocked` says *clear the gates first*, never *there is nothing else*. Dropping the loser's rows along with its verdict is how a report full of started-and-unfinished work proposes only gates, and precedence is the wrong tool for deciding what gets reported — it decides only what leads.
...
1. **Status** — `in-progress` before `open`. Started work outranks a high-priority item nobody has begun: finishing costs less than starting, and an item left half-done goes stale where an untouched one does not. This is what makes "finish before starting" actionable rather than advice.
