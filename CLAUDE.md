...
- **3 agents:** `<PREFIX>-dev` (design → implement → deploy → Reference Sync → handoff), `<PREFIX>-qa` (review + test → sign-off → handoff), `<PREFIX>-pm` (verify QA → write log → docs check → handoff; runs on `model: sonnet` — its work is mechanical close-out, so Opus is reserved for `<PREFIX>-dev`/`<PREFIX>-qa`).
...
- **Commands:** thin entry points that hand work to agents — `/code` and `/fix` start the pipeline at `<PREFIX>-dev`, others target a specific phase (roadmap, wrap-up, design). `/code` and `/fix` also own the top-level steps the subagents can't do: Step 0.5 verification capture (`AskUserQuestion` proposing 2–3 candidate verifications + "Other" for a custom one-liner), the pre-QA "Ensure verification stack" step (start a serve-env in background when a UX/E2E target is down — subagents can't hold servers), the user-gated UAT-defer when a typed verification can't run headless, and, with `--prod`, the post-sign-off prod deploy via `<PREFIX>-deploy`. Same `.md` (Claude Code) / `.toml` (Gemini CLI) shape across projects; concrete set evolves over time.
...
- **Source-of-truth files:** `governed-paths.conf` (path → skill ownership + `DEPLOY_PATHS`), `deploy-config.yaml` on `DEPLOY_OWNER` (unified env schema — every env declares exactly one of `run:` serve-env / `deploy:` ship-env plus `url`; `verify:` is a plain env name; typed verifications resolve to the first non-prod env, never prod), `docs/roadmap.md` (open items), `docs/project-log.md` (delivery log).
