...
If a question times out unanswered — the timeout is the user's own `askUserQuestionTimeout` setting; with none configured a question waits indefinitely, which is why a session that runs unattended needs one — split by risk:
...
**State the regression scope in that question, not just the tasks — with its price.** It is the one setting that moves mission cost by an order of magnitude: `full` re-covers every prior verification per task *and* on every fix cycle, so `--regression full` on a multi-task mission is a different order of spend from the pinned `smart` default. Quote the multiplier while the user can still change it: `full` measured **8–17×** a `smart` mission of the same size (two real missions on 2026-08-27, $380 and $180 against a $21.56 single-task baseline). When `.claude/pilot/state.json` exists, a headless shift has recorded its actual `total_cost_usd` there — quote that running per-task figure rather than the baseline. The user is present exactly once, here — pricing the run while they can still change it is the same reason **Budget** is asked up front. Report it; never gate on it.
...
| `live` | re-measured, and the proposal still holds | **the user**, on a recommendation to approve |
...
**(a0) Settle the verdicts this run produced** — run when Step 2 left any `drifted`, `live` or `judgment` verdict. The autonomous ones are already closed and recorded; this step is the single sitting where the rest get answered.
...
- **Emerged** — each deferral **with its route** (walked at prod in (a2), or still open on a named out-of-band trigger — "deferred" must never cover both facts), the roadmap ids each task's `<PREFIX>-dev` reported on its `Roadmap:` field, scope the mission filed itself, and follow-ups a failed task created. Every row names its home, per `code.md § Done`. On the standing mission add **`Next shift:`** — the top `max_tasks` runnable rows the next standing run would take, by rank, each with its id. It is the veto point: one word at a check-in changes an item's status or priority before a run nobody watches starts it.
...
**Closing line — staged by clearing cost.** `code.md § Done` derives the line from the Open rows, and its table assumes one command closes them. This lane routinely holds three kinds at once, so stage them by what it costs to clear one: **decisions** (a parked gate, an acknowledgement — minutes, no code) → **repairs** (a recorded `fail`, a gated data repair — one lane each) → **items** (in-progress work — a mission). Rows all in one stage → that stage's batched command, per the batch rule; for decisions that is `/pilot --gates`, which re-measures each one before proposing anything and closes the ones the measurement kills without asking. When every Open row is one a run can execute on its own, the batched command is **`/pilot` with no arguments** — the standing mission takes exactly this report's rows as its task list — so name that rather than restating the set; when those rows plus the runnable backlog outnumber what one mission takes, name the loop form instead, **`/loop 30m /pilot --max-tasks 1`** (a fixed interval, never self-paced — the only loop shape the workflow proposes anywhere), because that is the shape that works the whole queue rather than the first ten rows of it. Rows spanning stages → `Next:` names **stage one only**, with a second `Then:` line showing the path. **Never propose a stage a predecessor would dissolve:** a gate that releases two in-progress items changes what the mission should contain, so clear it first and say so rather than batching both into one line that will be wrong by the time it runs.
```
