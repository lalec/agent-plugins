...
.claude/commands/audit.md         ← from tpl-commands.md § /audit, substitute <PROJECT> and <PREFIX>
...
- [ ] `.claude/commands/code.md`, `fix.md`, `pilot.md`, `tweak.md`, `audit.md`, `revert.md`, `tidy.md`, `whats-up.md`, `roadmap.md`, `blueprint.md`, and `wrap.md` exist (markdown format, `$ARGUMENTS`)
- [ ] `blueprint.md` carries a `model:` line in its frontmatter, states that `docs/roadmap.md` is its only write, carries the three Step 2 probes (`Sound on the real corpus`) and both halves of the executor rule (`decision <n> contradicted`), and its Step 6 is one AskUserQuestion (Confirm + Decide); `roadmap.md § Rank` carries `Plans stay together`; `pilot.md` carries `Plan decisions:`, the `**Size:** medium` override and `follows the child, not the address`; `<PREFIX>-pm.md` step 2.5 carries `closed by its last child`
...
- [ ] `.claude/skills/<PREFIX>-review/` has SKILL.md + `references/code-review-reception.md`, `references/requesting-code-review.md`, `references/issuing-findings.md`, `references/security-review.md` and `references/vex.yaml` (no `verification-before-completion.md` — `<PREFIX>-debug` owns that). SKILL.md contains a `## Read Map` and **no** `## Reference Sync` section (static-content skill), plus an `## Update `references/vex.yaml` when:` checklist — `vex.yaml` is project state, the one file in this skill the project authors.
...
- [ ] That section defines **five** blocks in order (Verdict · Learned · Status · Open · Emerged), `Learned` capped at 3 bullets and admitting only what changes the reader's picture; Open's table is three columns with `Next` a runnable command, never a bare roadmap id, and `recommend:` on every decision row; Emerged's table is `Item | Home | Recommendation`, ranked, with the five recommendation words; the section carries `Nothing after the blocks`
...
- [ ] The allowance step ran: either `~/.claude/settings.json`'s `statusLine.command` contains `usage-snapshot.sh` with `refreshInterval` set and `~/.claude/usage-snapshot.sh` is byte-identical to the plugin's, or the user declined and the settings block was printed. `bash ~/.claude/usage-snapshot.sh --read` exits 0 (a figure, or `unknown`, both correct). `pilot.md` contains `usage-snapshot.sh --read` in its Step 0 read, its Step 2 stop and its `whats-up-store:` reads, and contains `A percentage is not a threshold`
...
- [ ] `.claude/commands/audit.md` carries both a `pilot-lane:` and a `whats-up-store:` block, routes the triage rule to `<PREFIX>-review/references/security-review.md § Triage` instead of restating it, caps Step 1 at 5 packages per run (whole packages, never a row count), records every candidate `under_investigation` in Step 0 before assessing any, and carries the critical-severity floor; `<PREFIX>-review/references/vex.yaml` exists and `governed-paths.conf` carries `DEPENDENCY_MANIFESTS` plus the `vex.yaml` `EXEMPT` row
- [ ] `<PREFIX>-review/references/security-review.md` carries `## Triage` (the four VEX statuses, the five justifications, the compensating-control rule) and a `## Escalate` naming the first push and the new-input-surface deploy plus the `git remote set-head` line — its checklist section A blocks on `affected`, not on high/critical
