...
5. **Reference Sync** — complete the Reference Sync checklist for each project (`<PREFIX>-*`) domain skill used (context is fresh now; docs must reflect what just changed). Third-party skills (e.g. `agent-browser`) are not project-managed — do not add or modify their Reference Sync sections. If the checklist reveals a reference file needs to be renamed, retired, or split, use **<PREFIX>-skill** skill before making structural changes (<PREFIX>-skill owns the lifecycle: SKILL.md, skill-manifest.md, and the file rename/delete).
