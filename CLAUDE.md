...
- **7 lifecycle skills:** `<PREFIX>-log`, `<PREFIX>-review`, `<PREFIX>-debug`, `<PREFIX>-deploy`, `<PREFIX>-test`, `<PREFIX>-skill`, `<PREFIX>-docs`. `<PREFIX>-design` is added only when a Frontend category is present. `<PREFIX>-deploy` is the single source of truth for *how* the project deploys — owns `deploy-config.yaml` and the gate/trigger/verify logic; agents, commands, and other skills delegate here. `<PREFIX>-dev` invokes it for non-prod (mid-pipeline, so QA tests a running stack); **prod deploy is invoked only at the command top level** (`/code|/fix --prod`), never from a subagent, because the gate needs `AskUserQuestion` and that only reaches the user at the top level.
