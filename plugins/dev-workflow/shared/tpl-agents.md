...
2.5. **Roadmap status update** — required, not best-effort: flip `**Status:**` to `done · YYYY-MM-DD` or `in-progress` on each item identified in step 1.5. If step 1.5 found no match, say so in `Notes:`. Do NOT add new entries — only advance existing ones. If an item you flipped to `done` carries a `**Parent:**` and no sibling naming that parent is still open (`python3 .claude/graph/graph.py roadmap-open` nests a plan's children under it; read the file when the script is absent), flip the parent to `done · YYYY-MM-DD` too — an umbrella is closed by its last child, never by hand. `docs/roadmap.md` is the only roadmap store: never mirror the status to an external tracker, and never report the absence of one as a finding.
