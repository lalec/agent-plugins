...
- The closing line carries "in a fresh session" — a completed pilot has consumed most of this one. A standing mission's closing line also states `runnable rows left: <N>`, and a loop that fires this command paces itself on that number alone: at 0, the longest idle it allows; above 0, the **shortest** delay it allows — the next task is already known, so an idle gap between tasks is throughput lost, not caution (the first real loop idled 25 minutes between 20-minute tasks). Rows the next tick will take are `Next shift` rows, never Open ones: nothing about them needs a person.
```
