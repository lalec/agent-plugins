...
1.5. **Roadmap entries** — before implementing, check if the task reveals scope to track:
   ...
   - **Optional mirror.** If a Notion MCP tool is actually available in this session, also create a page in the Roadmap database. If no such tool is present, **skip silently** — `docs/roadmap.md` is the source of truth and the mirror is a convenience, so nothing is lost and nothing is pending. Its absence is not a finding: never report it in the handoff, the delivery log, or as a manual-sync note. (Delete this sub-step at install time on projects with no Notion mirror.)
...
5. **Sign off** — only when review reports no unresolved findings and `<PREFIX>-test` reports clean. Map the outcome honestly:
   ...
   - **The acceptance statement's end state is unreached or unwalkable** → `Status: blocked`, naming the step that blocks. Never `signed-off-with-deferrals`: an unproven feature is not "clean except environment", and deferring the one check that proves the journey hands the user an untested feature with a checklist. A missing test environment is a reason to say so loudly here, not a reason to pass it downstream.
...
2.5. **Roadmap status update** — required, not best-effort: flip `**Status:**` to `done · YYYY-MM-DD` or `in-progress` on each item identified in step 1.5. If step 1.5 found no match, say so in `Notes:`. Do NOT add new entries — only advance existing ones. **Optional mirror:** if a Notion MCP tool is actually available in this session, use `notion-search` + `notion-update-page` to sync status; if none is present, **skip silently** — `docs/roadmap.md` is the source of truth, so the status is already recorded. Its absence is not a finding: never report it in the handoff, the delivery log, or as a manual-sync note. (Delete this clause at install time on projects with no Notion mirror.)
