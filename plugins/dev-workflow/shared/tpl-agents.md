...
- `regression_mode: smart` (default) — `<PREFIX>-test` runs Smoke + this task's verifications + prior verifications for the same files.
- `regression_mode: full` — `<PREFIX>-test` additionally runs every prior verification plus the full Regression suite.
...
3. **Test** — invoke `<PREFIX>-test`, passing through: `regression_mode`, the names of any new verifications captured for this task, and the paths `<PREFIX>-dev` reported under `Files changed:`. The skill selects tiers from those signals (Smoke always · this task's verifications always · prior verifications whose `paths` overlap the changed paths · Regression iff `regression_mode: full`) and runs the suite.
