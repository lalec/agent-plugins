...
| `/audit` | Turns your dependency scanner's output into a decision instead of a list. Every advisory gets one status — exploitable here, or not, with the reason why not from a fixed set — and only the exploitable ones block or become work, so a parser reading files strangers upload outranks a build tool with more advisories and a higher score. It works a package at a time rather than an advisory at a time, because a package's advisories usually share one sink and one version bump, and it writes down every advisory it did not get to, so a tree with a hundred undecided ones can never report clean. Each answer is kept, so the next run only looks at what is new: a project with eighty settled advisories costs a grep, not eighty judgement calls. A WAF or a rate limit in front of a flaw lowers its priority and never closes it, because those can be turned off and the flaw is still there. `--deep` hands the branch to Claude Code's own `/security-review`. |
...
| `/blueprint` | Plan on the strongest model, run on the cheaper ones. Give it a goal — or the id of a roadmap item that is only a name — and it reads what the project already knows, proves against the code that the fix it has in mind would actually work, decides everything an executor would otherwise have to guess, and writes the result to the roadmap as one umbrella plus child items sized for `/pilot`, each with its lane, its dependencies and the checks that prove it. Proving means more than the file existing: the line it picked is on the path your scheduler really takes, every caller passes what the change reads, the value it chose survives the data you already store, and the list it means to cover comes from the registry rather than from memory — so a plan that looked right on paper gets corrected here instead of by the agent building it. It edits nothing else; the last line is the `/pilot --items …` that runs the plan. |
...
Everything else is written automatically, inside your repo. Two things sit outside it and are offered rather than done: scheduling headless shifts, and letting runs see how much of your Claude allowance is left. The second wraps your status line so the figures it already receives are saved where `/pilot` can read them — one file per open session, reconciled on read, because a dozen sessions writing one file just publishes whichever of them rendered last. Your own status-line script isn't edited, and deleting one line from your settings undoes it. Say no and it prints the change for you to make yourself. It's per-machine, not per-project, so the next project you install into finds it already there.
...
Lanes are yours to extend: any command that declares how work should reach it becomes a destination, so a project-specific routine joins the rotation without touching `/pilot`. A lane that burns something finite — credits, API calls, generation runs — runs against a budget you grant at the same up-front gate, and stops dispatching when it's spent rather than quietly overrunning. A lane also says how an approved decision gets carried out, and the run proves it can do that *before* it asks you — so you're never asked to approve something the session then turns out to be unable to perform. A lane that can undo its own writes — it records a reversal before touching anything, and names the command that replays it — says that too, and then a decision the re-measure confirmed is carried out by the run and reported with the path that undoes it, instead of waiting for you.
...
What makes that safe to walk away from is that it asks nothing: the standing mission never gates, and a loop over named items asks once, on the first firing, then carries your answer to every later one. A question timeout in `/config` is not what keeps it moving — the workflow never waits on one. Auto-continue at the usage limit is worth switching on too, but don't rely on it for a loop — it resumes a conversation stopped mid-request, and a loop hitting the limit is usually asleep on a timer or waiting on an agent, which is why the fixed interval carries the weight. It has two other blind spots worth knowing, both measured on a real overnight run: when a batch of parallel helpers all run out at the same moment, each one's death reaches the session as its own message and each gets refused in turn, and Claude Code gives up after the second retry — so a wide batch can burn the whole retry budget in half a minute. And if the Mac sleeps through the reset, it waits for a keypress instead of carrying on; `caffeinate -i` while a long run is going prevents that, and the scheduled shifts already hold sleep off for themselves. This is why the run stopping itself cleanly, before it reaches the wall, is worth more than any retry. Start the session with Remote Control and the few questions a run does decide to ask — a decision whose evidence moved, something blocking a high-priority item, a task that failed twice — arrive on your phone as a push; answer in time and it's yours, miss it and it's parked. Everything else it could have asked is parked without asking. A check-in is `/whats-up` and then `/pilot --gates`, which puts the whole parked batch to you in one sitting; the report also lists the next shift's queue, so a word from you drops an item before a run nobody watches starts it.
...
It spends each run on a handful of packages rather than a handful of advisories, and takes each one whole. Thirteen advisories on one image library share a single sink and a single version bump, so deciding them row by row buys one fix and burns the run — while the multipart parser reading the same upload one step earlier, and the token library that let the request in, sit undecided. Anything a run doesn't reach is recorded as undecided rather than left unmentioned, so the count you see is the real one. And a critical in something that ships at runtime is looked at even when its package falls below the cut — which needs a scanner that reports severity, so the run tells you when yours doesn't instead of calling that part clear.
...
```
Ranks the open items — anything already started before anything untouched, then priority, then dependency order so nothing runs before the thing it needs, then category; a plan's children stay together under their umbrella rather than scattering through the list — and offers two ways forward: hand the top set to `/pilot` as one unattended mission, or start a single item through `/code`/`/fix` with its own gate. A filter narrows the set before ranking. Only the selecting happens here; the run limits, retries and budget all belong to `/pilot`.

### Plan something bigger than one task
```
/blueprint <goal that spans several tasks>
/blueprint <roadmap id that is only a name>
```
A goal handed straight to `/pilot` gets decomposed at the gate, on whatever model the session runs, with no time to read the record — so a plan written as prose steps becomes one oversized task, and the agent building it ends up choosing the model, the threshold and the budget itself. This lane does the planning first, on the strongest model, and writes the plan where `/pilot` reads it: one umbrella item carrying the origin, the context, and a numbered *decisions made here so the executor does not* block, plus child items each small enough for one unattended run — its lane, what it depends on, whether Sonnet or Opus should build it, the exact place in the code it lands, what *done* means, and the checks with concrete expected values — a check whose number will settle a later decision also records which rule produced it, because a share made of two rules can't be split apart afterwards and the run that measured it may not be repeatable. Every value it prescribes is tried against the code before it's written down, not just looked up: that the line it picked is on the path your scheduler actually takes and that every caller passes what the change reads; that the thing being replaced exists in one place and not five; that the regex, key format or threshold it chose survives the values already sitting in your database — and a list the change has to cover is read out of the registry that holds it rather than typed from memory. Where a probe can't run, the plan says so and makes the first check the thing that would have proved it. A mechanism the code already has is named for reuse rather than rebuilt; a lever the log shows was tried and reverted is refused unless there's new evidence. Decisions that are yours — money, product tradeoffs — come to you once, at the single gate, then get written as decided. An executor that finds a decision missing reports it rather than guessing; one whose own read of the code contradicts a decision fixes it the way the code allows and records what it saw, or — with nobody there to check the substitute — stops and says which decision the code refused. Its only write is the roadmap, committed and not pushed, and it ends with the `/pilot --items <umbrella> --no-push` that runs the plan in dependency order — or `/code <first item>` if you'd rather sit in on the first one. `/pilot` expands the umbrella to the children still open and hands each one the plan's decisions, which travel with the child however you start it; the umbrella closes itself when the last child does.

It's `/blueprint` rather than `/plan` because Claude Code's own `/plan` toggles plan mode, which designs one change for the session you're in; a blueprint is a plan a session with no memory of this one — or a shift nobody watches — can run. It runs on the model its frontmatter names (Fable); change that one line if your account doesn't have it.
...
| **How wide it goes** | Decided after the code is written, from what actually changed. A change crossing two domains, or landing where a check is already failing, pulls in **more** checks — named ones, and it says which. It never quietly pulls in all of them: re-running the whole suite is an order-of-magnitude decision, so only you can ask for it with `--regression full`, and a run that has it tells you the price before it starts. Within the normal scope the checks are ordered by how much they're really at risk — a check anchored to a file that half your suite touches is weaker evidence than one anchored to two files — and walked against a budget. Anything the budget doesn't reach is **recorded as unproven with the reason**, so the next run raises it rather than it quietly counting as passed. |
| **When it can't run** | Recorded *blocked*, with the reason and what would close it — never as a pass. A check whose condition never arose didn't pass; it didn't run. An environment that answers at no address — a batch job that runs once and exits — is passed over for the next one rather than treated as the thing to check, so a component with a job in front of its dev server still gets verified against the dev server. |
...
| **Open** | Your inbox: anything that needs a decision or an action from you now — a question waiting on you, a check that failed, something you said you'd verify live, work left in the tree, work you started and haven't finished. Each row carries the exact thing to run or click and, where it needs a decision, the one the run would take, so overturning it is one word, and something already written to the roadmap still shows up here if it's what you should do next. A choice made for you because you didn't answer in time appears here, labelled as such. Nothing open says `None`. |
| **Emerged** | The receipt: everything else the run turned up, ranked, each with where it now lives and what the run recommends — queue it at this priority, fold it into an item that already exists, hold it until a named trigger, keep or discard a stash. Ten findings read as the two that matter and the eight that can wait; the low ones share a line and none is dropped. Nothing appears in both blocks, and anything with nowhere to live goes in Open rather than quietly evaporating. |
...
`done` and `proven` are deliberately different words. Code that was written is *done*; a journey somebody actually walked is *proven* — and a change that shipped with a check nothing could run says `not proven`, rather than reading as finished. Long runs get more rows, never more prose: rows in a good state fold into a single line, Open never folds, and Emerged folds only its low rows. Anything the run wants to tell you after the blocks has to earn a row: a caveat that changes what you'd do is an Open row with a recommendation, one that changes what you believe is a Learned bullet, and the rest is cut.
